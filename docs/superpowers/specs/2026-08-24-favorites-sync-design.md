...
- 账号范围变化时立即发布新范围的本地集合；旧账号已在途的请求只能更新旧账号缓存，不能再发布到当前界面，下一轮改为同步新账号。
...
每轮远端请求携带启动该轮时的用户 ID。请求返回时，只有当前范围仍是同一账号才更新界面发布值；如果期间退出或切换账号，只更新原账号的隔离缓存，再为最新范围补跑一轮，防止旧账号收藏短暂泄露到新账号界面。
...
11. 同步请求进行中切换账号时，旧账号结果不会发布到新账号界面。
12. 退出后当前集合切回游客，另一个账号看不到前一个账号缓存。
13. 删除账号成功后只清除目标账号的本地缓存和队列。
